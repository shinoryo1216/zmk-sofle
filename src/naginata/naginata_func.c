...
// ここから下：システム③（Colemak+薙刀式）L1・R6の3機能キー用に追加。
// input_unicode_hexは既存のngh_JKG（U+FF08＝（）等と全く同じ仕組みを使う。
void ng_lt() { // ＜ U+FF1C
    input_unicode_hex(F, F, N1, C);
}

void ng_gt() { // ＞ U+FF1E
    input_unicode_hex(F, F, N1, E);
}

void ng_colon_full() { // ： U+FF1A
    input_unicode_hex(F, F, N1, A);
}

void ng_slash_full() { // ／ U+FF0F
    input_unicode_hex(F, F, N0, F);
}

void ng_question_noenter() { // ？（確定改行なし）
    raise_zmk_keycode_state_changed_from_encoded(LS(SLASH), true, timestamp);
    raise_zmk_keycode_state_changed_from_encoded(LS(SLASH), false, timestamp);
}

void ng_exclaim_noenter() { // ！（確定改行なし）
    raise_zmk_keycode_state_changed_from_encoded(LS(N1), true, timestamp);
    raise_zmk_keycode_state_changed_from_encoded(LS(N1), false, timestamp);
}

// L6/R1ホールドで発火するIME強制切替（絶対指定）。
// トグル式の半角/全角キーではなく、force-set系のキーコードのみを送る設計にすることで
// 2台目キーボードによるIME状態のズレの影響を受けないようにする（設計ガイド4章参照）。
// Windows/Linuxは基本的にLANG1/LANG2をそのまま送るが、Linux(fcitx5)側は
// 設定ガイド4-2の通りfcitx5側で「LANG1=Mozcへ直接切替」「LANG2=キーボード-英語(US)へ直接切替」
// をアクティブ化ショートカットとして個別登録しておく必要がある。
void ng_os_ime_japanese() {
    switch (naginata_config.os) {
    case NG_WINDOWS:
    case NG_LINUX:
    case NG_IOS:
